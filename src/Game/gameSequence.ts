...
export async function startGame(): Promise<string> {
    const name: string = await ask.question(chalk.blue('Hello Adventurer, what is your name? ')) as unknown as string;
    ...
    while (true) {
        const confirmPlayAgain: string = await ask.question('Would you like to play again? Type Yes or No: ') as unknown as string;
...
export async function gameSequence(user: User): Promise<void> {
    ...
    console.log(
        chalk.black.bgWhite.bold(`You are in the ${user.getCurrentRoom}.\n Inventory: ${user.getInventory}`)
    );
    ...
    const userChoice: string = await ask.question(chalk.blue(`Check instructions, Show status, Or make a move.\n`)) as unknown as string;
    ...
    } else if (itemInRoom && userChoice === foundItem) { // Check if the user wants to pick up the item in the room
    ...
    } else if (userChoice === 'Show Instructions') { // Check if the user wants to see the instructions again
        console.log('Pulling up instructions scroll\n');
        await user.showInstructions();
    } else if (userChoice === 'Show Status') { // Check if the user wants to see their status
        console.log('Pulling up status scanner now');
        await user.showStatus();
    } else { // If the user input is not valid, prompt them to try again
